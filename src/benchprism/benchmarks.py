...
from .metrics import checker_classeval, checker_io, checker_testbench
  ...
  def load_for_test_generation(self, lang: str) -> Seq[Snippet]:
    ...
    for file in sorted((self._data_dir / 'source_file_parser').iterdir()):
      ...
      snippets.extend([Snippet(id=self._make_id(row), data={
          ...
          'checker': checker_testbench,
          'source_code': row['source_code'],  # redundant for checker
      }) for row in data])
    return snippets
